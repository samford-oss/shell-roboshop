...
do
    ...
    then
        IP=$(aws ec2 describe-instances --instance-ids i-0bfec147b3b4e7669 --query "Reservations[0].Instances[0].PrivateIpAddress" --output text)
    else
        IP=$(aws ec2 describe-instances --instance-ids i-0bfec147b3b4e7669 --query "Reservations[0].Instances[0].PublicIpAddress" --output text)
    fi
